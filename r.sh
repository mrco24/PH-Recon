...
python3 /root/install-tools/tools/github-search/github-subdomains.py -t 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 -d $domain > gitsub.txt
...
curl -s https://dns.bufferover.run/dns?q=.$domain |jq -r .FDNS_A[]|cut -d',' -f2|sort -u | tee bufferoversub.txt
...
-connect $domain:443 ) ) | grep -Po '((http|https):\/\/)?(([\w.-]*)\.([\w]*)\.([A-z]))\w+' | tee altnamesub.txt
...
domain_ip(){
gf ip /root/recon/$domain/subdomain/finalsub.txt | sort -u > root/recon/$domain/subdomain/ip_sub.txt
}
domain_ip
